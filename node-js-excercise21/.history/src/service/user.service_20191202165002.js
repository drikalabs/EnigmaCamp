...
class UserService{
    ...
    async findByCriteria(criteria){
        let data = await getConnection.userRepository(User)
        .createQueryBuilder('users')
        .where(`users.username like '%${criteria.username}%' || users.email like '%${criteria.email}%'`)
        .Where(`users.email like '%${criteria.email}%'`)
        .Where(`users.fullname like '%${criteria.fullname}%'`)
        .getMany();
        return data;
    }
}
